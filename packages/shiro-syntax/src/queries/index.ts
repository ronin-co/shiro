...
export const getBatchProxy = (
  ...
  try {
    ...
    // Always restore the original value of `IN_SHIRO_BATCH`, even if `operations()`
    // throws. This is essential, otherwise `IN_SHIRO_BATCH` might stay outdated.
...
const serializeValue = (
  ...
  if (typeof value === 'function') {
    // Temporarily store the original value of `IN_SHIRO_BATCH`, so that we can resume it
    // after the nested function has been called.
    const ORIGINAL_IN_SHIRO_BATCH = global.IN_SHIRO_BATCH;

    // Since `value()` is synchronous, `IN_SHIRO_BATCH` should not affect any other
    ...
    try {
      ...
      // Always restore the original value of `IN_SHIRO_BATCH`, even if `value()` throws.
      // This is essential, otherwise `IN_SHIRO_BATCH` might stay outdated.
